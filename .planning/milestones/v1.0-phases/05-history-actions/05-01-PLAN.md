...
must_haves:
  ...
  artifacts:
    ...
    - path: "components/HistoryCard.tsx"
      ...
      contains: "handleCopy"
    - path: "components/TranscriptDetailActions.tsx"
      provides: "Client component with copy and delete buttons for detail page"
      contains: "handleCopy"
    - path: "app/history/[id]/page.tsx"
      provides: "Imports TranscriptDetailActions for copy and delete on detail page"
  key_links:
    ...
    - from: "components/TranscriptDetailActions.tsx"
      to: "/api/transcript/delete"
      via: "fetch POST in handleDelete"
      pattern: "fetch.*api/transcript/delete"
    - from: "app/history/[id]/page.tsx"
      to: "components/TranscriptDetailActions.tsx"
      via: "imports and renders TranscriptDetailActions"
      pattern: "TranscriptDetailActions"
---
...
2. Create `components/TranscriptDetailActions.tsx` (client component):
   - `"use client"` directive
   - Props: `{ transcriptId: string, segments: TranscriptSegment[] }` (import TranscriptSegment from `@/lib/types`)
   - Import `useRouter` from `next/navigation`, `toast` from `sonner`, `formatTranscriptText` from `@/lib/format`, `Button` from ui, `Copy`, `Trash2`, `Check` from `lucide-react`, and AlertDialog parts from ui
   - Add `useState` for `copied` (boolean) and `deleting` (boolean)
   - `handleCopy`: calls `formatTranscriptText(segments, false)` then `navigator.clipboard.writeText(text)`, shows toast, sets copied with 2s timeout
   - `handleDelete`: POSTs to `/api/transcript/delete` with `{ ids: [transcriptId] }`, on success toasts and calls `router.push("/history")`, on error toasts failure
   - Renders a row of copy + delete buttons. Delete wrapped in AlertDialog with "Delete transcript?" title and permanent warning.

3. Update `app/history/[id]/page.tsx`:
   - Import `TranscriptDetailActions` from `@/components/TranscriptDetailActions`
   - Render `<TranscriptDetailActions transcriptId={transcript.id} segments={transcript.segments} />` between the video metadata section and the TranscriptViewer
   - The page stays a server component; TranscriptDetailActions handles all client interactivity
